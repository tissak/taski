//! Property test for the pure write-path linchpin `rewrite_done_date`
//! (ADR-0012). This is the "never corrupts a line" contract for the done-date
//! rewrite, mirroring `rewrite_scheduled_proptest.rs` byte-for-byte with
//! `'⏳'` (U+23F3) → `'✅'` (U+2705).
//!
//! Property: for an arbitrary line and an arbitrary (valid-date-or-None)
//! `desired`, `rewrite_done_date` **never panics** and:
...
//!     `extract_done_date(s)` reflects `desired` on a stamp;
...
/// Count raw `✅` (U+2705) occurrences, by bytes (independent of the impl's
/// char-based count).
fn done_emoji_count(s: &str) -> usize {
    let needle = "\u{2705}".as_bytes();
...
fn expected_rewrite(line: &str, desired: Option<&str>) -> Option<String> {
    ...
    match desired {
        ...
        Some(date) => match (hc, token) {
            (0, None) => Some(format!("{line} \u{2705} {date}")),
            (1, Some((_, end))) => {
...
proptest! {
    ...
    #[test]
    fn rewrite_done_date_never_corrupts(
        line in ".*",
        desired in prop::option::of(valid_date()),
    ) {
        let result = rewrite_done_date(&line, desired.as_deref());
        ...
        match &result {
            RewriteResult::Rewritten(new_line) => {
                ...
                match desired.as_deref() {
                    Some(d) => {
                        prop_assert_eq!(done_emoji_count(new_line), 1, "stamp leaves exactly one ✅");
                        let parsed = extract_done_date(new_line);
                        prop_assert_eq!(
                            parsed.as_deref(),
                            Some(d),
                    ...
                    None => {
                        prop_assert_eq!(done_emoji_count(new_line), 0, "clear leaves no ✅");
                        prop_assert!(
                            extract_done_date(new_line).is_none(),
                            "no done date after clear"
                        );
                    }
                }
            }
            RewriteResult::Unchanged => {
                ...
                match desired.as_deref() {
                    Some(d) => {
                        let parsed = extract_done_date(&line);
                        prop_assert_eq!(
                            parsed.as_deref(),
                            Some(d),
