//! Property test for the pure write-path linchpin `rewrite_scheduled`
//! (ADR-0009 Phase 2). This is the "never corrupts a line" contract for the
//! scheduled-date rewrite, mirroring the parser's "never panics" proptest and
//! the daemon's write-back "never corrupts" proptest.
//!
//! Property: for an arbitrary line and an arbitrary (valid-date-or-None)
//! `desired`, `rewrite_scheduled` **never panics** and:
...
//!     `⏳` (one after a mark, zero after an unmark), and
//!     `extract_scheduled_date(s)` reflects `desired` on a mark;
...
/// Count raw `⏳` (U+23F3) occurrences, by bytes (independent of the impl's
/// char-based count).
fn hourglass_count(s: &str) -> usize {
    let needle = "\u{23F3}".as_bytes();
...
fn expected_rewrite(line: &str, desired: Option<&str>) -> Option<String> {
    ...
    match desired {
        ...
        Some(date) => match (hc, token) {
            (0, None) => Some(format!("{line} \u{23F3} {date}")),
            (1, Some((_, end))) => {
...
proptest! {
    ...
    #[test]
    fn rewrite_scheduled_never_corrupts(
        line in ".*",
        desired in prop::option::of(valid_date()),
    ) {
        let result = rewrite_scheduled(&line, desired.as_deref());
        ...
        match &result {
            RewriteResult::Rewritten(new_line) => {
                ...
                match desired.as_deref() {
                    Some(d) => {
                        prop_assert_eq!(hourglass_count(new_line), 1, "mark leaves exactly one ⏳");
                        let parsed = extract_scheduled_date(new_line);
                        prop_assert_eq!(
                            parsed.as_deref(),
                            Some(d),
                    ...
                    None => {
                        prop_assert_eq!(hourglass_count(new_line), 0, "unmark leaves no ⏳");
                        prop_assert!(
                            extract_scheduled_date(new_line).is_none(),
                            "no scheduled date after unmark"
                        );
                    }
                }
            }
            RewriteResult::Unchanged => {
                ...
                match desired.as_deref() {
                    Some(d) => {
                        let parsed = extract_scheduled_date(&line);
                        prop_assert_eq!(
                            parsed.as_deref(),
                            Some(d),
